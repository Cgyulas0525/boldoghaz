...
<div class="form-group col-sm-6">
    <div class="form-group col-sm-12">
        ...
        @if (!isset($constructionphase))
            {!! Form::text('name', null, ['class' => 'form-control','maxlength' => 100]) !!}
        @else
            @if ( !is_null($constructionphase->parent_id))
                {!! Form::text('name', null, ['class' => 'form-control','maxlength' => 100]) !!}
            @else
                {!! Form::text('name', null, ['class' => 'form-control', 'readonly' => 'true', 'maxlength' => 100]) !!}
            @endif
        @endif
    </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var table = $('.partners-table').DataTable({
                ...
                columns: [
                    {
                        title: '<a class="btn btn-primary" title="Felvitel" href="{!! route('cpChildCreate', isset($constructionphase->id) ? $constructionphase->id : 0 ) !!}"><i class="fa fa-plus-square"></i></a>',
                        data: 'action',
                        sClass: "text-center",
                        width: '200px',
                        name: 'action',
